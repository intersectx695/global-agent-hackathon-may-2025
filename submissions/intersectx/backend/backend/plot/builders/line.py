...
class LineBuilder(IBuilder):
    ...
    @staticmethod
    def _build_line_plot(data, title=None, x=None, y=None, company_name=None, **kwargs):
        ...
        fig = px.line(
            ...
            markers=True,  # Add markers to line for better data point visibility
            line_shape="spline",  # Smooth curves for more modern look
            **kwargs,
        )

        # Enhanced styling for lines and markers
        fig.update_traces(
            line=dict(width=2.5),  # Slightly thicker lines
            marker=dict(
                size=8,
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                ...
                zeroline=False,
                rangeslider=dict(visible=False),  # No range slider by default
            ),
            yaxis=dict(
                showgrid=True,
                gridcolor="#F0F0F0",
...
# Test function to demonstrate line chart
if __name__ == "__main__":
    import os
    ...
    async def main():
        ...
        # Create test data - monthly growth data
        ...
        line_builder = LineBuilder(netlify_agent)

        # Generate the plot
        url = await line_builder.plot(chart_data, company_name="Datagenie AI")
        print(f"\nLine chart generated and uploaded to: {url}")
